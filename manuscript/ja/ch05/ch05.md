...
> **注** （数学者へのエクスキューズ——$d^2=0$ と全微分可能性）本書では「$f$ が全微分可能なら混合偏微分が可換、ゆえに $d(df)=0$」という順序で進めた。しかし微分形式の公理的な立場では、**$d^2=0$ こそが $d$ の定義の一部**であり、$d(df)=0$ は定理ではなく要請である。逆に言えば、$d^2=0$ を課すことが「混合偏微分の対称性が成り立つ関数クラス（＝全微分可能な関数）を相手にしている」という宣言に相当する。どちらの順序で学んでも、両者が同値な条件として噛み合うことに変わりはない。
