...
第1章では、$${dx, dy, dz}$$ を「ベクトルを1つ食べてスカラーを返す横ベクトル（$${1}$$-form）」として定義し、$${\int f dx}$$ を行列作用の極限として読み直した。本章ではこれを拡張し、<strong>2つのベクトルを食べる面積測定器（</strong>$${2}$$<strong>-form）</strong>と<strong>3つのベクトルを食べる体積測定器（</strong>$${3}$$<strong>-form）</strong>を、同じ部品 $${dx, dy, dz}$$ から代数的に組み立てる。
...
- $${k=2}$$：$${2}$$-form は二重ベクトル（<strong>向き付きの面積</strong>）を測る。成分3つ、大きさは $${\sqrt{A_{yz}^2+A_{zx}^2+A_{xy}^2}}$$
- $${k=3}$$：$${3}$$-form は三重ベクトル（<strong>向き付きの体積</strong>）を測る。成分1つ、大きさは絶対値
