...
このように、$d$ が作用するたびに測定器の次数が $0\to1\to2\to3$ と一つずつ上がっていく。$d$ は、場を新しい矢印場として作り替えるのではなく、係数の局所的な変化を読み取り、次数の一つ高い測定器として出力する。

#### 8.1.2 測定器はそのまま、新たな場を考える——$\nabla$ の立場

一方、第7章で導入した $\nabla = \begin{pmatrix} \frac{\partial}{\partial x} & \frac{\partial}{\partial y} & \frac{\partial}{\partial z} \end{pmatrix}^T$ の世界では、**測定器（$\nabla$）は変わらず、作用する相手に応じて新たな場が生まれる**。$\nabla$ はつねに同じ演算子だが、スカラー場 $f$ に作用すればベクトル場 $\nabla f$ を、ベクトル場 $\mathbf{F}$ に作用すればスカラー場 $\nabla\cdot\mathbf{F}$ やベクトル場 $\nabla\times\mathbf{F}$ を返す。
...
ベクトル場 $\mathbf{F} = (F_x, F_y)^T$ は実空間に住んでいる。その測定器 $\omega = F_x\,dx + F_y\,dy$ を、パラメータ空間の用紙に**引き戻す**。引き戻し（pullback）とは、実空間の測定器をパラメータ空間の言葉に焼き直す操作だ。写像 $\phi: (r,\theta) \to (x,y)$ で実空間に送るのと逆向きに、測定器を戻してくる。$dx = \cos\theta\,dr - r\sin\theta\,d\theta$, $dy = \sin\theta\,dr + r\cos\theta\,d\theta$ を使って、
...
測るグリッドの形は均一なまま考えられる。場所ごとの換算係数は pullback と $\ast$ が引き受け、極限操作は最後にまとめて行えばよい。
...
> - $\int_{\partial M}\omega = \int_M d\omega$ 一本で、微積分学の基本定理・ストークス・ガウスの定理を共通の構造として理解できる。
