...
本章は、この本の**ハイライト**である。しかし、その実あっさりと、これまでのどの章よりも短くなる。そのために第1章から第7章までの入念な準備があった——行列、ウェッジ積、外微分、計量、ホッジ・スター、そしてベクトル解析。これらの道具がすべて揃っているからこそ、本章ではただ「翻訳する」だけで、二つの世界が一つの絵に収まる。我々は二つの言語を手に入れた——測定器の微分（$d$）と、場の微分（$\nabla$）。この二つは、やっていることが根本的に違う。しかし積分を通じて繋がるとき、まったく同じ結果を与える。その仕組みを、これから徹底的に掘り下げる。
...
このように、$d$ が作用するたびに測定器の次数が $0\to1\to2\to3$ と一つずつ上がっていく。核心は**場はそのまま、測定器の側が変化する**ことだ。$f$ はスカラー場のままだが、$d$ が作用するたびに新しい測定器——$df$（$1$-form）、$d\omega$（$2$-form）、$d\eta$（$3$-form）——が生み出される。

#### 8.1.2 測定器はそのまま、新たな場を考える——$\nabla$ の立場

一方、第7章で導入した $\nabla = \begin{pmatrix} \frac{\partial}{\partial x} \\ \frac{\partial}{\partial y} \\ \frac{\partial}{\partial z} \end{pmatrix}$ の世界では、**測定器（$\nabla$）は変わらず、作用する相手に応じて新たな場が生まれる**。$\nabla$ はつねに同じ演算子だが、スカラー場 $f$ に作用すればベクトル場 $\nabla f$ を、ベクトル場 $\mathbf{F}$ に作用すればスカラー場 $\nabla\cdot\mathbf{F}$ やベクトル場 $\nabla\times\mathbf{F}$ を返す。
...
ベクトル場 $\mathbf{F} = \begin{pmatrix} F_x \\ F_y \\ F_z \end{pmatrix}$ に $\nabla\cdot$ を作用させると、スカラー場が得られる。
...
$\nabla\times\mathbf{F}$ は**縦ベクトル**、$d\omega$ は**$2$-form（反対称行列）**、そして $\ast d\omega$ は**$1$-form（横ベクトル）**である。回転は「外微分で次数を上げてから、ホッジ・スターで次数を戻す」という二段階の操作だ。
...
$\nabla\cdot\mathbf{F}$ は**スカラー場**、$\ast d\ast\omega$ も**$0$-form（スカラー場）**である。発散は「ホッジ・スターで $2$-form に変え、外微分で $3$-form に上げ、再びホッジ・スターでスカラーに落とす」という三段階の操作だ。
...
場そのものには手を触れない。$d$ が作用するたびに、**新しい測定器が生み出される**。
...
$d$ はつねに測定器の次数を $+1$ する。この立場では、**場を測る手段そのものが変わる**。$f$ を測る $df$ と $\omega$ を測る $d\omega$ は異なる種類の測定器であり、混同されることは原理的にない。

積分は、作った測定器を領域に適用するだけである。そして $\int_{\partial M}\omega = \int_M d\omega$ が、測定器のすげ替えと積分の間の整合性を保証する。

#### 8.5.2 方法②（∇の方法）——測定器はそのまま、新たな場を考える

$\nabla$ という測定器はつねに同じ。作用する相手に応じて、**異なる種類の場が結果として生まれる**。
...
方法①と方法②は、やっていることが**根本的に違う**。一方は場をそのままに新しい測定器を生み出し、他方は測定器をそのままに新たな場を考える。操作方法も、出てくる対象の種類も異なる。
...
ベクトル場 $\mathbf{F} = \begin{pmatrix} F_x \\ F_y \end{pmatrix}$ は実空間に住んでいる。その測定器 $\omega = F_x\,dx + F_y\,dy$ を、パラメータ空間の用紙に**引き戻す**。引き戻し（pullback）とは、実空間の測定器をパラメータ空間の言葉に焼き直す操作だ。写像 $\phi: (r,\theta) \to (x,y)$ で実空間に送るのと逆向きに、測定器を戻してくる。$dx = \cos\theta\,dr - r\sin\theta\,d\theta$, $dy = \sin\theta\,dr + r\cos\theta\,d\theta$ を使って、

$$\tilde{\omega} = F_r\,dr + (r F_\theta)\,d\theta$$

が得られる。

> **注** （正規直交成分と form 係数）ベクトル解析では $\mathbf{F}$ を「長さ1の矢印」に対する成分 $(F_r, F_\theta)$ で表す。しかし微分形式の自然な基底は座標微分 $(dr, d\theta)$ であり、$d\theta$ 方向の目盛り間隔は場所によって $r$ 倍異なる。したがって $\tilde{\omega} = F_r\,dr + \tilde{F}_\theta\,d\theta$ と書いたときの form 係数 $\tilde{F}_\theta$ は $r F_\theta$ であり、ベクトル解析の成分 $F_\theta$ とは次元も値も異なる。この違いが、方法①と方法②の公式の見た目の差を生んでいる。
