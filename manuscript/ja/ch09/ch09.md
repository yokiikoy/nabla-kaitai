...
> **注** （原点では？）$\rho = 0$ では $\omega$ が定義できない。原点を含む全空間で扱うには、通常の関数としての発散ではなく、分布（$\delta$ 関数）や積分形式で点電荷を表す必要がある。本書ではそこには立ち入らず、原点を除いた領域での計算に限ることにするが、積分形式（ガウスの定理）を使えば、右辺に $\rho/\varepsilon_0$ が現れることを第10章で正式に扱う。
