...
しかし、これだけの道具を揃えて、使わずに終わるのはもったいない。本章では、ここまでに構築した辞書を実際の問題に適用し、ベクトル解析の難問を機械的に解いてみせる。理論は第8章までで十分だ——ここから先は<strong>演習</strong>である。
...
1. <strong>ヤコビ行列 $J$ を書く</strong>。座標変換 $x = x(q_1,q_2,q_3)$, $y = y(q_1,q_2,q_3)$, $z = z(q_1,q_2,q_3)$ から偏微分を並べる。$J$ の各列はパラメータ空間の各軸方向の一歩が実空間でどう動くかを表す。

2. <strong>計量 $\mathbf{g} = J^T J$ を計算する</strong>。これは第6章 §6.1.3 以来の手順だ。$\mathbf{g}$ の対角成分が各軸方向の目盛りの二乗、非対角成分が軸どうしの直交度を表す。

3. <strong>$\mathbf{g}$ から $\ast$ の辞書を導き、$d$ と組み合わせる</strong>。$\mathrm{grad} = d$、$\mathrm{rot}=\ast d$、$\mathrm{div}=\ast d\ast$ の各公式に、その座標系での $\ast$ の辞書を代入すれば、求める公式が機械的に得られる。

> <strong>注</strong> （測定器の略記と $\sharp/\flat$ ）注意してほしいのは、ここでの $\mathrm{grad} = d, \mathrm{rot} = \ast d, \mathrm{div} = \ast d \ast$ は、あくまで「測定器側」の略記だという点である。通常のベクトル解析の $\mathrm{grad}$ や $\mathrm{rot}$ は矢印としてのベクトル場を返すため、微分形式側から結果を戻すには計量 $\mathbf{g}$ による $\sharp$（シャープ）変換が必要になる。直交デカルト座標ではこの変換は成分レベルでは見えないが、曲線座標ではスケール因子やホッジ・スターの辞書にその効果が明確に現れる。

> <strong>注</strong>（座標 coframe と正規直交 coframe）
> 曲線座標では、座標そのものに対応する $1$-form と、実際の長さを1単位として測る $1$-form を区別する必要がある。前者を<strong>座標 coframe</strong>、後者を<strong>正規直交 coframe</strong> と呼ぶ。ホッジ・スターの辞書は、これら二つの coframe を繫ぐスケール因子の情報をすべて引き受けている。これにより、我々は「歪んだ座標系」の上で「真っ直ぐな計算」を行うことができるのである。
...
> <strong>注</strong>（円柱座標の 4 層構造）
> ベクトル場を正しく微分形式として扱うために、以下の 4 つのレイヤーを区別しよう。
>
> | 名称 | 表示 | 円柱座標での具体例 |
> | :--- | :--- | :--- |
> | <strong>座標 coframe</strong> | $dq^i$ | $dr, d\theta, dz$ |
> | <strong>正規直交 coframe</strong> | $\theta^i$ | $dr, r d\theta, dz$ |
> | <strong>ベクトル物理成分</strong> | $F_i$ | $F_r, F_\theta, F_z$ |
> | <strong>form 係数</strong> | $\tilde{F}_i$ | $F_r, rF_\theta, F_z$ |
>
...
> <strong>注</strong> （$\tilde{\omega}$ のチルダについて）
...
> <strong>注</strong>（球座標の 4 層構造）
> 
> | 名称 | 表示 | 球座標での具体例 |
> | :--- | :--- | :--- |
> | <strong>座標 coframe</strong> | $dq^i$ | $d\rho, d\theta, d\phi$ |
> | <strong>正規直交 coframe</strong> | $\theta^i$ | $d\rho, \rho d\theta, \rho\sin\theta d\phi$ |
> | <strong>ベクトル物理成分</strong> | $F_i$ | $F_\rho, F_\theta, F_\phi$ |
> | <strong>form 係数</strong> | $\tilde{F}_i$ | $F_\rho, \rho F_\theta, \rho\sin\theta F_\phi$ |
>
...
> <strong>注</strong> （電磁気学が未習の読者へ）以下に出てくる「電場」や「点電荷」といった用語は、理解できなくても構わない。単に「$\frac{1}{\rho^2}$ に比例するベクトル場」の計算だと思って眺めてほしい。第10章でも、電磁気学そのものを深く掘り下げるわけではない。ただ、マクスウェル方程式を微分形式で書くと恐ろしく簡潔になること、そして「みんなここで美しいと言って終わるが、行列まで落とすとどうなるか」を実際に全部書き下す——それだけだ。
