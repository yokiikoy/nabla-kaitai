...
二つの縦ベクトル $\mathbf{a} = (a_x, a_y, a_z)^T$, $\mathbf{b} = (b_x, b_y, b_z)^T$ の**ドット積**（内積）を次で定義する。
...
> **注** （グリーンの定理）$S$ が $xy$ 平面内の領域 $D$ で、$\mathbf{F} = (P, Q, 0)^T$ の場合、ストークスの定理は次の形になる。これはグリーンの定理と呼ばれる。
>
