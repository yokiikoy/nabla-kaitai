...
二枚のデカルト座標を使う。左が実空間 $(x,y)$、右がパラメータ空間 $(r,\theta)$ だ。右の用紙には、どこも同じ大きさの正方形グリッドが敷き詰められている。

このグリッドを左の実空間に送ったとき、扇形は描けない。なぜなら、まだ内積（計量）を導入していないので、角度の情報が使えないからだ。$\theta$ 方向の換算は $\Delta\theta$ という数のまま扱われ、弧長 $r\Delta\theta$ にはならない。実際に起きているのは、マス目どうしの<strong>換算係数が場所によって変わる</strong>ということだけだ。$r$ が大きい場所では $\Delta\theta$ の換算係数 $r$ が大きく、$r$ が小さい場所では小さい。

ベクトル場 $\mathbf{F} = (F_x, F_y)^T$ の測定器 $\omega = F_x\,dx + F_y\,dy$ を右の用紙に<strong>引き戻す</strong>。この引き戻し（pullback）が、マス目ごとに異なる換算係数を自動的に織り込んでくれる。あとは右の用紙の上で正方形グリッドのまま $\ast d\ast\omega$ を計算すれば、

$$\ast d\ast\omega = \frac{1}{r}\frac{\partial}{\partial r}(r F_r) + \frac{1}{r}\frac{\partial F_\theta}{\partial \theta}$$

が得られる。測るのはつねに均一な正方形グリッド、場所ごとの換算係数の違いは pullback が代数的に吸収する。ヤコビ行列 $J$ と計量 $g=J^T J$ がこの換算をすべて担う。グリッドは有限でよい——極限は最後の一手で十分だ。
