...
ここまで長かった。第1章で $dx$ を行ベクトルと定義し、第2章で面積計と体積計を組み立て、第3章で積分を再構築し、第4章で外微分 $d$ を得て、第5章で計量 $g$ とホッジ・スター $\ast$ を手に入れ、そして第6章ではついに $\nabla$ を正面から定義してベクトル解析を一気に展開した。
...
#### 7.5.1 方法①——場はそのまま、測定器の側が変化する（$d$）

場そのものには手を触れない。$d$ が作用するたびに、<strong>新しい測定器が生み出される</strong>。

- スカラー場 $f$ の変化を測りたい → $df$ という $1$-form 測定器を作る
- $1$-form 場 $\omega$ の循環を測りたい → $d\omega$ という $2$-form 面積計を作る
- $2$-form 場 $\eta$ の湧き出しを測りたい → $d\eta$ という $3$-form 体積計を作る
...
> - 方法①（$d$）：場はそのまま、測定器の側が変化する。新しい測定器が生まれる。
> - 方法②（$\nabla$）：測定器（$\nabla$）はそのまま、新たな場を考える。
