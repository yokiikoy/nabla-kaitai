...
そのあと、分割を細かくしていく。有限の比は微分係数になり、有限の行列式は偏微分でできた行列式になる。この極限で得られる標準的な測定器の作り替えを、数学では **引き戻し** と呼び、$\Phi^*$ と書く。
...
これは標準的な記法ではない。有限区間上で「実空間側の測定器を、時間側の区間で同じ値を返すように作り替える」操作を、本書では一時的にこう書く。
...
が得られる。ここで初めて、標準的な引き戻し $\gamma^*$ という記号を使う。つまり、本章では $\gamma^*$ を先に知っているものとして使うのではなく、有限区間版 $\gamma_h^\square$ から発見する。

以後、2 次元・3 次元で有限のマス目や箱に対して同じことを行うときは、有限セル版の作り替えを $\Phi_h^\square$ と書く。これは、いま 1 次元で作った $\gamma_h^\square$ を多次元へ拡張した記号である。

> **注**（$\Phi_h^\square$ は標準的な引き戻しそのものではない）
>
> $\gamma_h^\square$ や $\Phi_h^\square$ は、本書で有限区間・有限セル上の測定操作を区別するために用意した記法である。標準的な微分形式論で用いられる引き戻し $\Phi^*$ そのものではない。
>
> 標準的な引き戻し $\Phi^*$ は、点ごとの微分、すなわち接写像 $D\Phi$ を用いて定義される。これはすでに $h\to0$ の極限を取った後の世界である。一方、本書で $\Phi_h^\square$ と書いているものは、その手前にある有限差分の測定操作である。
>
...
>
> したがって、$\Phi_h^\square$ は標準的な $\Phi^*$ を置き換えるものではない。有限セル上の測定値をセル幅で正規化し、分割幅 $h\to0$ の極限を取ることで、標準的な引き戻し $\Phi^*$ の係数が得られる。
>
...
**④ 具体例の引き戻しを書く。** 極限を取ったあと、時間軸上での測定器は
...
次数が上がるごとに行列式のサイズが一つずつ大きくなる。この構造は $n$ 次元の一般の $k$-form でもまったく同じである。$k$-form の標準的な引き戻しとは、**偏微分からなる $k \times k$ 行列式で、$k$ 次元の測定器をパラメータ空間へ焼き直す操作**にほかならない。
