...
<strong>② 素朴な置き換え——そのままでは合わない。</strong> 質点の位置を $x = \gamma(t)$ と書く。速度は $v(t)=x'(t)$ である。仕事の測定器は $F\,dx$ ——これは実空間の変位を食べて仕事の小片を返す $1$-form だ。
...
$1$-form なら $1 \times 1$ 行列式（座標関数の微分 $x'(t)$ 等）、$2$-form なら $2 \times 2$ 行列式、$3$-form なら $3 \times 3$ 行列式（$J$）——次数が上がるごとに、つじつま係数を司る行列式のサイズが一つずつ大きくなる。上の偏微分行列式の式こそが <strong>$3$-form の引き戻しの一般的な定義</strong> である。
