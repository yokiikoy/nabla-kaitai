...
中心力場では $L$ が保存されるから、面積速度も一定——これがケプラーの第二法則であり、角運動量保存則の幾何学的表現である。ここで引き戻しが角運動量保存則を生んでいるわけではない。保存則は中心力場という物理的仮定から来る。引き戻しがしているのは、同じ面積を $(x,y)$ ではなく $(r,\theta)$ で計算しても同じ量として読めるように、測定器側の係数を合わせることである。
...
正しい値に一致した。積分値を保つために、測定器 $dx \wedge dy \wedge dz$ は $(r,\theta,z)$ 空間では $r\,dr \wedge d\theta \wedge dz$ に作り替えられなければならなかったのである。§4.1 の $\gamma'(t)$、§4.2 の $r$、§4.3 の $r$（一般には $J$）は、保存則の原因ではない。同じ仕事・面積・質量を別のパラメータや座標で計算しても同じ値として読めるようにする、測定器側のつじつま係数である。
...
3. **第4章（本章）**：曲線・曲面・領域に形式を適用し集計する（積分）。引き戻しにより「同じ量を別の変数で読んでも積分値が変わらないように測定器を作り替える」という統一原理を、仕事・面積速度・質量の三つの具体例を通じて確立した
