...
> **注** （$\Phi_h^\square$ は本書の記法）$\gamma_h^\square$ や $\Phi_h^\square$ という記号は、本書で有限区間・有限セル上の測定操作をはっきり区別するために用意した記法である。一般的な教科書では、この記号を置かず、有限の差分比や小さな面積要素を見たあと、すぐに微分係数やヤコビアンの式へ進むことが多い。つまり、他書がまったく別のことをしているわけではない。本書は、その途中段階に名前を付けて、有限セル上で何を測っているのかを見えるようにしている。
