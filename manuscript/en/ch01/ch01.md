...
We have now extended the one-variable expression $df=f'(x)\,dx$ with $dy$ and $dz$ as measuring devices of the same type. In the concrete example below and in the extension to three dimensions in §1.2.6, we will see how all three devices come into play.
...
is not merely a computational symbol. It can be read as the form obtained by rebuilding the measuring device $dx$ on the $x$ axis into a measuring device on the $t$ axis built from $dt$.
...
The question is: the measuring device $dx$, which measures the $x$-component in physical space, appears as what kind of measuring device on the parameter-space side?
...
In this way, the pullback is the operation of rebuilding a measuring device in physical space so that it returns the same value on another scale of variables.
...
In this chapter we leaned on the $x$ direction for <strong>concrete examples of integrals and</strong> $df$, and often treated $y$ and $z$ as fixed “slices.” Even so, the linear forms in physical space are the full trio $dx$<strong>,</strong> $dy$<strong>, and</strong> $dz$ (§1.2.3).
