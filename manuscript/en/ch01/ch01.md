...
Near the end of this chapter, we want you to experience once, by your own hand, what it means to “write in another coordinate system” as an actual calculation. We have not yet defined a change of coordinate system as a general theory here, but in §1.4.3 we already saw that **the same** $dx$ **can have different matrix representations depending on how coordinates are chosen**. This exercise is meant to let you confirm that feeling by calculation. **Why do we want you to do these calculations before assembling all the precise language of coordinate changes?** So that when formulas become suddenly busy in later chapters, you already have the gut feeling that “of course the components change.”
...
(The row above is the row vector that acts on displacements in parameter space, constructed in §1.4.3. It is **not** the same thing as stacking the Cartesian display $\begin{pmatrix}1&0&0\end{pmatrix}$ from §1.1.3 as a column vector on the right.)

Answer the following questions.

<strong>Question 1</strong>  
Suppose a particle is at the point $P(r=2,\theta=\pi/6,z=0)$ in cylindrical coordinate space. The particle undergoes a small displacement of $0.1$ in the $r$ direction. **In this section, we denote by** $\mathbf{v}$ **the vector** $\begin{pmatrix}\Delta r\\\Delta\theta\\\Delta z\end{pmatrix}$ **listing the changes in the parameters** $r,\theta,z$ (this ordering is different from the Cartesian column $\begin{pmatrix}\Delta x\\\Delta y\\\Delta z\end{pmatrix}$ in §1.0–§1.2). Write this displacement vector $\mathbf{v}$ in terms of its components.
...
(In cylindrical coordinates, the components of $dx$ depend on $r$ and $\theta$, so what we mean here is the row evaluated at the coordinates of the point $P$. The notation $dx(\mathbf{v})$ for applying it to $\mathbf{v}$ is the same as since §1.1.3.)

What matters is that the second component is generally the coefficient $-r\sin\theta$, and thanks to $r$ it **carries a length dimension**. Substituting $r=2$ and $\theta=\pi/6$ gives the numerical value $-1$ for that coefficient, but **the number** $-1$ **by itself does not carry a dimension**. The correct reading is that the matrix entry $-r\sin\theta$ carries the length dimension. This is the key to dimensional analysis.
...
Here, **the second component of the input vector** $0.1$ is an angle (dimensionless), but **the coefficient corresponding to the second column of the matrix**, $-r\sin\theta$, carries a length dimension. Therefore the product $-0.1$ as output has the dimension of length.
...
> - The protagonist of Chapter 1 is reading $dx$ as a matrix / linear form and reading $\int f\,dx$ as the limit of actions. The main exposition leaned on the $x$ direction, but the Cartesian measuring devices $dy$ and $dz$ were defined in the same way as $dx$ in §1.2.3.
> - $df$ is also unified as a row vector; extension to several dimensions connects straightforwardly as an increase in the number of components.
...
In this chapter we leaned on the $x$ direction as the main line for <strong>concrete examples of integrals and</strong> $df$, and in many places suppressed $y$ and $z$ as “cross sections.” Even so, the linear forms in physical space are the full trio $dx$<strong>,</strong> $dy$<strong>, and</strong> $dz$ (§1.2.3).

In the next chapter we combine these three to introduce the <strong>wedge product (exterior product</strong> $\wedge$<strong>)</strong>, constructing <strong>area-measuring devices and volume-measuring devices (</strong>$2$<strong>-forms,</strong> $3$<strong>-forms)</strong>, and move on to <strong>higher differential forms</strong> that are not limited to the type that act on vectors and return scalars. Aggregation along curves, surfaces, and regions (line integrals, surface integrals, volume integrals) will be treated in later chapters, after these measuring devices are in place.

<!-- role: roadmap -->
In later parts we proceed to the <strong>exterior derivative</strong> $\mathrm{d}$, the <strong>Hodge star operator</strong> $\ast$, $\mathrm{grad}$ ($\nabla$), $\mathrm{curl}$ ($\nabla\times$), $\mathrm{div}$ ($\nabla\cdot$) of vector analysis, Stokes’s theorem, Maxwell’s equations, and the basic equations of fluid mechanics. The correspondence between differential forms and vector fields in three dimensions can be organized using the Hodge star.
