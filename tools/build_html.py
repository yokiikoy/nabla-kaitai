...
def process_markdown(markdown_text):
    ...
    for i, line in enumerate(lines):
        ...
        if line.strip().startswith('>'):
            ...
            # GFM allows continuation of quote without > on every line, but for safety in this 
            # text-heavy manuscript, we close quote if the line doesn't start with >
            if line.strip() == '':
                close_quote()
    ...
    if footnotes:
        ...
        for fn_id, content in footnotes.items():
            formatted_content = protector.restore(apply_inline_formatting(content))
