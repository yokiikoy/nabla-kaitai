...
> <strong>注</strong> （数学者へのエクスキューズ）我々は今、平然と $\mathbf{v}_1^T M \mathbf{v}_2$ （横 $\times$ 行列 $\times$ 縦）という書き方をした。厳密な純粋数学の流儀では行儀が悪い。ベクトルを横に寝かせる裏に、本書ではまだ定義していない「内積（計量）」が入り込みうるからだ。<strong>しかし、我々は物理学者だ。</strong> 計算には支障がないのだから、割り切って進もう。
...
> <strong>注</strong> （二次形式とクロス積）二次形式としての面積と、3成分ベクトルとしてのクロス積のあいだには、<strong>ホッジ双対</strong>と呼ばれる関係が示唆される。詳しい対応と、これが必然かつ奇跡であることは後の章で改めて明らかにするとして、ここでは「向き付き面積の情報の持ち方の違い」程度に留めておこう。
...
なお、実はこの3つのブロックは、§2.4.4–2.4.5 で構成した<strong>面積計の行列そのもの</strong>となり、対応は次のとおりだ：
