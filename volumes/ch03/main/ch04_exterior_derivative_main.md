...
これまでの旅路を振り返ろう。第1章では、積分記号の末尾にへばりつく `dx` を「微小量」から「行列」へと解体し、それを一次形式（1-form）と呼んだ。第2章では、その物差し `dx, dy, dz` を掛け合わせる<strong>ウェッジ積（∧）</strong> という魔法の工具を手に入れ、面積計（2-form）と体積計（3-form）を組み立てた。第3章では、これらの局所的「測定器」を曲線・曲面・領域という現場に持ち込み、引き戻しという翻訳アルゴリズムを使って集計（積分）する方法を確立した。
...
我々は既に強力な代数的ツール、<strong>ウェッジ積（∧）</strong> を持っている。そして、0-formに対する `d` の動作は $df = f_x dx + f_y dy + f_z dz$ で、これはあたかも $f$ と $dx, dy, dz$ の「積」を取って、$f$ を偏微分で「ひっぺがした」ように見える。
...
ここに現れた係数 $A_x + B_y + C_z$ は、2-formの係数 $(A, B, C)$ の「発散（div）」に他ならない。<strong>外微分 `d` は、2-formを3-formへ昇格させるとき、自動的に「発散」を計算してくれる</strong>のである。

#### 4.2.4 第2章の直感との整合性
第2章で我々は、2-formを「2つのベクトルの絡み合いを測る反対称行列」と定義した。外微分 `d` が1-formから作り出した2-form $d\omega = (R_y - Q_z) dy\wedge dz + \dots$ は、まさにそのような反対称行列である。その成分 $(R_y - Q_z, P_z - R_x, Q_x - P_y)$ は、ベクトル解析の回転が持つ「ある方向の微小な輪に沿った循環の密度」という物理的直感（ストークスの定理の萌芽）を完璧に反映している。`d` の定義に用いたライプニッツ則とウェッジ積は、この物理的直感を<strong>代数的に、矛盾なくエンコードするための必然的な設計</strong>だったのだ。

> <strong>【ここまでのチェックポイント】</strong>
> - 一般の1-form $\omega = P dx + Q dy + R dz$ に `d` を作用させると、2-form $(R_y - Q_z) dy\wedge dz + (P_z - R_x) dz\wedge dx + (Q_x - P_y) dx\wedge dy$ が得られる。これはベクトル場の<strong>回転（rot）</strong> に対応。
> - 一般の2-form $\eta = A dy\wedge dz + B dz\wedge dx + C dx\wedge dy$ に `d` を作用させると、3-form $(A_x + B_y + C_z) dx\wedge dy\wedge dz$ が得られる。これは<strong>発散（div）</strong> に対応。
...
1.  <strong>0-form（関数）への作用</strong>： $df = \frac{\partial f}{\partial x}dx + \frac{\partial f}{\partial y}dy + \frac{\partial f}{\partial z}dz$。
2.  <strong>線形性</strong>： $d(\omega_1 + \omega_2) = d\omega_1 + d\omega_2$。
3.  <strong>積の法則（次数付きライプニッツ則）</strong>： $\omega$ が $k$-formのとき、$d(f \omega) = (df) \wedge \omega + f \wedge (d\omega)$。特に、$f$ が0-formなので $df$ は1-formであり、ウェッジ積 $\wedge$ によって $\omega$ と正しく結合される。
4.  <strong>定数係数形式の微分</strong>： $d(dx) = d(dy) = d(dz) = 0$。（より正確には、座標関数 $x, y, z$ から作られる1-form $dx, dy, dz$ の外微分は0。）

これらのルールだけから、前節までのすべての計算が再現できる。ルールは天下りではなく、0-formに対する自然な拡張（$df$）と、形式の代数（ウェッジ積）との整合性から「発見」されたものなのである。

#### 4.3.2 魔法の呪文：$d^2 = 0$
これらのルールから導かれる、最も重要かつ美しい性質がある。それは <strong>「外微分を2回続けて作用させると、必ず0になる」</strong> というものだ。記号で書けば、任意の $k$-form $\omega$ に対して
$$ d(d\omega) = 0. $$
これを略して $d^2 = 0$ と書く。これは単なる計算の便法ではない。<strong>ベクトル解析の二大恒等式 $\mathrm{rot}(\mathrm{grad}\, f) = 0$ と $\mathrm{div}(\mathrm{rot}\, \mathbf{F}) = 0$ を、一言で言い表す大原理</strong>なのである。
...
$$ d(df) = 0. $$
<strong>$d^2 f = 0$ が成り立つ根本理由は、混合偏微分の対称性（ヤングの定理）にある。</strong> ウェッジ積の反対称性 $(dy \wedge dx = -dx \wedge dy)$ と、偏微分の対称性がちょうど打ち消し合って、すべての項が相殺されるのである。
...
$d^2=0$ の深い意味は、第7章で学ぶ<strong>ストークスの定理</strong> $\int_M d\omega = \int_{\partial M} \omega$ と結びつくと明らかになる。ここで $\partial M$ は領域 $M$ の境界である。この定理と $d^2=0$ を組み合わせると、
$$ \int_M d(d\omega) = \int_{\partial M} d\omega = \int_{\partial (\partial M)} \omega. $$
しかし左辺は $d^2=0$ より0である。一方、幾何学における基本的な事実として「<strong>境界の境界は常に空集合</strong>」($\partial (\partial M) = \emptyset$) がある。右辺は空集合上の積分なので0となり、式は矛盾なく成立する。$d^2=0$ は、この「境界の境界はない」という位相幾何学的事実の、微分形式による代数的な反映なのである。

#### 4.3.6 物理学への示唆
$d^2=0$ が保証する $\mathrm{rot}(\mathrm{grad}\, f)=0$ と $\mathrm{div}(\mathrm{rot}\,\mathbf{F})=0$ は、物理学において<strong>保存場</strong>や<strong>ソレノイド場</strong>の数学的基礎を与える。例えば、静電場 $\mathbf{E}$ が保存力場ならば $\mathrm{rot}\,\mathbf{E}=0$ であり、それはある電位 $V$ を使って $\mathbf{E} = -\mathrm{grad}\, V$ と書けることを意味する。また、磁場 $\mathbf{B}$ についてのガウスの法則 $\mathrm{div}\,\mathbf{B}=0$ は、$\mathbf{B}$ が常にあるベクトルポテンシャル $\mathbf{A}$ を使って $\mathbf{B} = \mathrm{rot}\,\mathbf{A}$ と表現できる可能性を示唆する（これにはさらに位相的考慮が必要だが）。外微分の枠組みは、これらの関係を $d^2=0$ という単一の原理で統一的に理解する扉を開く。

> <strong>【ここまでのチェックポイント — 第4章全体】</strong>
> - 外微分 `d` の計算は、<strong>0-formへの作用</strong>、<strong>線形性</strong>、<strong>次数付きライプニッツ則</strong>、<strong>$dx,dy,dz$ の微分は0</strong>という4つのルールに基づく。
> - `d` は0-formを1-form（勾配）に、1-formを2-form（回転）に、2-formを3-form（発散）に昇格させる。これにより、ベクトル解析の三大微分演算子が一つの演算子の異なる側面として統合される。
> - <strong>$d^2 = 0$</strong> は外微分の最も重要な性質。これは混合偏微分の対称性に起因し、幾何学的には「境界の境界はない」ことの反映である。
...
本章で我々は、静的な測定器である微分形式を「変化方向」へと押し上げる動的操作、<strong>外微分 $d$</strong> を手に入れた。これにより、スカラー場から勾配が、ベクトル場から回転と発散が、代数的かつ系統的に導出できるようになった。しかも、それらの間に成り立つ重要な関係（$\mathrm{rot}\,\mathrm{grad}=0$, $\mathrm{div}\,\mathrm{rot}=0$）は、たった一つのエレガントな原理 $d^2=0$ に集約された。

しかし、ここで一つ大きな疑問が残る。我々の目の前には、<strong>0-form（スカラー）、1-form（ベクトルに相当）、2-form（ベクトルに相当）、3-form（スカラー）</strong> という、異なる種類の「実体」が混在している。物理学の教科書では、勾配はベクトル、回転もベクトル、発散はスカラーとして、すべて同じ「ベクトル解析」の枠組みで語られる。我々の微分形式の世界では、これらは異なる階数の対象として別々に存在している。<strong>この溝をどのように埋めれば、見慣れたベクトル解析の風景が現れてくるのだろうか？</strong>

その答えが、次章で学ぶ<strong>ホッジ・スター（$\ast$）</strong> である。この演算子は、微分形式の世界とベクトル解析の世界との間の「翻訳辞書」のようなものだ。$\ast$ を使えば、我々が今手にした $d$ の魔法が、そのまま勾配・回転・発散というおなじみの道具として翻訳され、ベクトル解析の公式の森を鮮やかに切り拓く道標となる。第II部「ナブラの裏の顔」の核心へ、いよいよ足を踏み入れるときが来た。