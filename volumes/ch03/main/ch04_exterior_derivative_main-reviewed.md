...
これまでの旅路を振り返ろう。第1章では、積分記号の末尾にへばりつく `dx` を「微小量」から「行列」へと解体し、それを**1-形式**（1-form）と呼んだ。第2章では、その物差し $dx, dy, dz$ を掛け合わせる**ウェッジ積（$\wedge$）** という魔法の工具を手に入れ、面積計（2-form）と体積計（3-form）を組み立てた。第3章では、これらの局所的「測定器」を曲線・曲面・領域という現場に持ち込み、引き戻しという翻訳アルゴリズムを使って集計（積分）する方法を確立した。
...
我々は既に強力な代数的ツール、**ウェッジ積（$\wedge$）** を持っている。そして、0-formに対する $d$ の動作は $df = f_x dx + f_y dy + f_z dz$ で、これはあたかも $f$ と $dx, dy, dz$ の「積」を取って、$f$ を偏微分で「ひっぺがした」ように見える。
